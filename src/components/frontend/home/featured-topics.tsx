    ...
    <section className="container px-4 md:px-6 py-12 mx-auto">
      <div className="mb-10 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-lg">
            Top selections sorted by detailed conversion analysis
          </p>
        </div>
      </div>
      ...
      {topics.length === 0 ? (
        <div className="text-center py-12 bg-white/30 dark:bg-slate-900/30 rounded-3xl border border-dashed border-muted-foreground/30 backdrop-blur-sm">
          <p className="text-muted-foreground text-xl">No featured topics found yet.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {topics.map((topic, index) => (
            ...
            >
              <Card className="flex items-center gap-5 p-5 border-0 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 ring-1 ring-black/5 dark:ring-white/10 group-hover:ring-amber-500/50 overflow-hidden relative h-full">
                ...
                <div className="absolute inset-0 bg-linear-to-r from-amber-500/0 via-amber-500/0 to-amber-500/0 group-hover:from-amber-500/5 group-hover:via-transparent group-hover:to-transparent transition-all duration-500 pointer-events-none" />
                ...
                <div className="flex-1 min-w-0 relative z-20 pointer-events-none">
                  <h3 className="text-xl font-bold text-foreground group-hover:text-amber-600 transition-colors line-clamp-2 mb-2">
                  ...
                  {topic.productCount > 0 && (
                    <div className="flex items-center gap-2">
                      <span className="px-2.5 py-0.5 rounded-full bg-amber-500/10 text-amber-600 text-xs font-semibold">
                      ...
                      {topic.authors.map((author, index) => (
                        <React.Fragment key={author.id}>
                          ...
                          >
                            ...
                            <span className="text-xs font-medium text-foreground group-hover/author:text-amber-600 transition-colors">
                              {author.name}
                            </span>
                          </Link>
                        </React.Fragment>
                ...
                <div className="absolute right-5 opacity-0 group-hover:opacity-100 transform translate-x-4 group-hover:translate-x-0 transition-all duration-300 text-amber-500 pointer-events-none">

...
export interface TopicWithImage {
  id: number
  name: string
  ...
  productCount: number
  excerpt?: string | null
  authors?: { id: string | number; name: string; slug: string; avatarUrl?: string | null }[]
}
...
export function NodeTopics({ topics, nodeId, basePath = '/gear' }: NodeTopicsProps) {
  return (
    <section className="animate-scale-up relative">
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-lg">Detailed guides and product analysis</p>
        </div>
      </div>
      ...
      {topics.length === 0 ? (
        <div className="text-center py-12 bg-white/30 dark:bg-slate-900/30 rounded-3xl border border-dashed border-muted-foreground/30 backdrop-blur-sm">
          <p className="text-muted-foreground text-xl">No related topics found yet.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {topics.map((topic) => (
            <div key={topic.id} className="block group h-full">
              <Card className="flex items-center gap-5 p-5 border-0 bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 ring-1 ring-black/5 dark:ring-white/10 group-hover:ring-primary/50 overflow-hidden relative h-full">
                {/* Main Topic Link covering the card */}
                <Link
                  href={
                    nodeId ? `${basePath}/${topic.slug}?n=${nodeId}` : `${basePath}/${topic.slug}`
                  }
                ...
                <div className="absolute right-5 opacity-0 group-hover:opacity-100 transform translate-x-4 group-hover:translate-x-0 transition-all duration-300 text-primary pointer-events-none">
